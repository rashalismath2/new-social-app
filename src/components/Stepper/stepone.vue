<template>
  <v-expansion-panels>
    <v-expansion-panel
      ...
      :key="i"
    >
      <v-expansion-panel-header class="black--text text--darken-4 headline">{{item.title}}</v-expansion-panel-header>
      <v-expansion-panel-content>
        <p class="mr-5">-<span>{{item.description}}</span></p>
        <div v-if="item.problems">
            <p class="black--text title">The problems you might have</p>
            <li class="black--text" v-for="p in item.problems" v-bind:key="p">
                -{{p}}
            </li>
        </div>
        <div v-else>
            <p class="black--text title">The cases you need help of</p>
            <li class="black--text" v-for="p in item.cases" v-bind:key="p">
                -{{p}}
            </li>
        </div>
        <v-btn class="btn-next" color="primary mt-5" @click="setStep(item.code,2)">
                  Continue
        </v-btn>
      </v-expansion-panel-content>                
    </v-expansion-panel>
...
export default {
    methods: {
        setStep(code,step){
            this.$emit("selectstep",[code,step])
        }
    },
    data() {
        return {
            categories:[
                    ...
                    { 
                         code:"s",
                        title:"School Psychology Consultant",
                        problems:null,
